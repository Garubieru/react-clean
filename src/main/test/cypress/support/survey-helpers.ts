export enum IconStatus {
  thumbsUp = 'data:image/png;base64,',
  thumbsDown = 'data:image/png;base64,',
}

export const testSurvey = (
  index: number,
  question: string,
  day: string,
  month: string,
  year: string,
  didAnswer: boolean,
): void => {
  cy.getByTestId('surveys-list')
    .children()
    .eq(index)
    .within(() => {
      cy.getByTestId('survey-question')
        .should('have.text', question)
        .getByTestId('survey-day')
        .should('have.text', day)
        .getByTestId('survey-month')
        .should('have.text', month)
        .getByTestId('survey-year')
        .should('have.text', year)
        .getByTestId('icon-image')
        .should(
          'have.attr',
          'src',
          didAnswer ? IconStatus.thumbsUp : IconStatus.thumbsDown,
        );
    });
};
